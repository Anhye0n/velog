...
router.post('/user/register', function (req, res, next) {
    ...
    crypto.randomBytes(64, (err, buf) => {
        crypto.pbkdf2(password, buf.toString("base64"), 100, 64, 'sha512', (err, key) => {
            ...
            let check_id_sql = "select EXISTS (SELECT * FROM user_info where id=? limit 1) as id_exist;"

            conn.query(check_email_sql+check_id_sql, [email, id], function (err, result, field) {
                console.log(result)
